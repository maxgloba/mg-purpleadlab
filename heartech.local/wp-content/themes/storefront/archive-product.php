...
<section class="product-section" id="product-section">
	<div class="container">
		...
		<article class="single-product">
			...
			<?php
				$images = get_field('product_slider', 5);
				if( $images ):
			?>
			<div class="slider">
				...
				<ul class="row">
					<?php $i=0; foreach( $images as $image ): $i++; ?>
						<li class="col-xs-4 <?php if($i == 1){ echo 'active'; } ?> ">
			...
			<div class="dashboard"><?php the_field('product_accent_box', 5); ?></div>
			<div class="infoboard"><?php the_field('product_info_box', 5); ?></div>
...
?>
<li class="product-section__product-item">
	<?php
	/**
	...
	if ($is_offset) {
		echo '<a class="button scroll-btn customBoxDisplay" data-template="product_'.$post_id.'" >ORDER NOW</a>';
		echo '
				...
				<div class="container">
					...
					<div class="s-offer-section__dscr-wrap">
						...
						<div class="s-offer-section__dscr-col">
							'.$of_description.'
							<ul class="subscribe-btns products">
		';
								if( have_rows('btn_green') ): while ( have_rows('btn_green') ) : the_row(); ?>
									<li><a href="/quiz/woo/?add-to-cart=<?php the_sub_field('product_id'); ?>" class="button accent-btn alert product_type_simple add_to_cart_button ajax_add_to_cart" rel="nofollow" data-quantity="<?php the_sub_field('product_quantity'); ?>" data-product_id="<?php the_sub_field('product_id'); ?>" aria-label="Add “<?php the_sub_field('product_name'); ?>” to your cart">YES I WANT</a></li>
								<?php endwhile; endif; ?>

								<?php if( have_rows('btn_light') ): while ( have_rows('btn_light') ) : the_row(); ?>
									<li><a href="/quiz/woo/?add-to-cart=<?php the_sub_field('product_id'); ?>" class="button s-offer-section__no-s-offer-link alert product_type_simple add_to_cart_button ajax_add_to_cart" rel="nofollow" data-quantity="<?php the_sub_field('product_quantity'); ?>" data-product_id="<?php the_sub_field('product_id'); ?>" aria-label="Add “<?php the_sub_field('product_name'); ?>” to your cart">NO, I DON’T WANT</a></li>
								<?php endwhile; endif;
		echo '
							</ul>
						</div>
