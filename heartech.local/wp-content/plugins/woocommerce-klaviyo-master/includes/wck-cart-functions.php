...
function wck_rebuild_cart() {
  ...
  $kl_cart = unserialize(base64_decode($utm_wck_rebuild_cart));
  $kl_cart = json_decode($kl_cart, true);
...
function wck_insert_checkout_tracking($checkout) {
  ...
  foreach ( $cart->get_cart() as $cart_item_key => $values ) {
    ...
    $event_data['$extra']['CartRebuildKey'] = base64_encode(serialize(json_encode($wck_cart)));
  ...
  WCK.trackStartedCheckout = function () {
    _learnq.push(["track", ("\$started_checkout"), $encoded_data ]);
  };
  var email = "{$email}";
  if (email) {
  ...
  if (jQuery) {
   ...
   jQuery('input[name="billing_first_name"]').change(function () {
     var email = jQuery('input[name="billing_first_name"]').val();
     if (email){
     var elem = jQuery(this),
        first_name = jQuery.trim(elem.val());
       _learnq.push(["identify", {"first_name" : first_name}]);
     }
   });

   jQuery('input[name="billing_last_name"]').change(function () {
     var email = jQuery('input[name="billing_email"]').val();
     if (email){
        var elem = jQuery(this),
        last_name = jQuery.trim(elem.val());
        _learnq.push(["identify", {"last_name" : last_name}]);
     }
   })
  }
