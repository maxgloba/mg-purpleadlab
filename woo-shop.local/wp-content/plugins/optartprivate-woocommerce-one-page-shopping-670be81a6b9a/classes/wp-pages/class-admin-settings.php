...
class admin_settings extends common
{
    ...
    public function register_plugin_options()
    {
        ...
        $defaults = array('add-text' => 0, 'singular-form' => 'item', 'plural-form' => 'items', 'force-refresh' => false, 'disable-scroll' => false);
        add_settings_field(
            $setting_id.'-add-text',
        ...
        add_settings_field(
            $setting_id.'-disable-scroll',
            $this->get_translation('label.'.$setting_id.'-disable-scroll'),
            array( $this, 'render_checkbox' ),
            self::get_plugin_identifier(),
            $section_id,
            array(
                'id' => $setting_id.'-disable-scroll',
                'value' => $this->get_option($options,$setting_id.'-disable-scroll', $defaults['disable-scroll']),
                'tiptip' => $this->get_translation('tiptip.'.$setting_id.'-disable-scroll')
            )
        );
    }
