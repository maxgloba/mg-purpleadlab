 ...
 * Tags: wpmenu,responsive,woocommerce
 * Version: 1.1.50
 */
...
function wooinstant_enqueue_scripts(){
	...
	wp_enqueue_style('wooinstant-stylesheet', plugin_dir_url( __FILE__ ) . 'assets/css/styles.css' );

	if( $wiopt['wi-drawer-direction'] == '1' ){
    	wp_enqueue_script( 'drawer-left-right', plugin_dir_url( __FILE__ ) . 'assets/js/drawer-left-right.js', array('jquery'), '', true );
	}else{
		wp_enqueue_script( 'drawer-right-left', plugin_dir_url( __FILE__ ) . 'assets/js/drawer-right-left.js', array('jquery'), '', true );
	}

    wp_enqueue_script( 'wi-ajax-script', plugin_dir_url( __FILE__ ) . 'assets/js/wi-ajax-script.js', array('jquery'), '', true );
	
	if( $wiopt['wi-disable-quickview'] != 1 ){
		wp_enqueue_script('wi-ajax-quick-view.js', plugin_dir_url(__FILE__) . 'assets/js/wi-ajax-quick-view.js', array('jquery'), '', true);
	}
...
function wooinstant_admin_notice_warn() {
	if ( !class_exists( 'WooCommerce' ) ) { ?>
	    <div class="notice notice-warning is-dismissible">
	        <p>
	        	<strong><?php esc_attr_e( 'Wooinstant requires WooCommerce to be activated', 'wooinstant' ); ?></strong>
	        </p>
	    </div> <?php
    }
